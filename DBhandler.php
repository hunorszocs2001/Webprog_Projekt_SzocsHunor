...
class DBhandler
{
    ...
    function __construct()
    {
        ...
        $username = "root";
        $database_name = "dbname=webprojekt";
    ...
    {
        $password = md5($password);
        $stmt = $this->dbh->prepare("SELECT * from user WHERE email='$email' AND password='$password'");
        $stmt->execute();
        if ($stmt->rowCount() > 0) {
            date_default_timezone_set('Europe/Bucharest');
            $date = getdate();
            $today = date("Y.m.d - H:i:s");
            $sql = "UPDATE user SET last_login=? WHERE email='$email' AND password='$password'";
